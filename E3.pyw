...
Label(primera_raiz,text="Seleccione su archivo y espere a que sea cargado").grid(row=1,column=0,sticky="e")
...
def licencia():
    messagebox.showinfo("Sobre el programa","Programa desarrollado por Ezequiel Elias en mayo del 2020. Contacto: infometeoba.blogspot")
...
def precipitaciones():  #Botón ENVIAR de Frame 1- Valores mensuales Opción PRECIPITACIONES
    ...
    if opcionPrinc.get()==2:     #Elige Valores mensuales - opción Precipitaciones
        ...
        if graficoP.get()==1:   #Elige graficar los datos
            ...
            plt.xlabel("Año")
            plt.ylabel("Precipitaciones (mm)")
            plt.xticks(rdo[:].index,rotation="vertical")
